...
## 2026-09-16 实现修订：独立第三方扩展

- Harness 已从内置扩展目录抽离到 `public/scripts/extensions/third-party/living-state-harness`，由独立 `manifest.json` 加载。
- 浏览器端只通过 SillyTavern 的公开前端模块和 Connection Manager 工作，不需要修改服务端路由或安装额外 npm 依赖。
- 设置键仍为 `extension_settings.livingStateHarness`；聊天中的状态快照、归档摘要、运行记录和提示词注入键均保持原值，现有聊天无需迁移。
- 模板加载使用完整第三方扩展 ID，核心模块导入按第三方目录深度解析。单元测试直接引用抽离后的实现，防止仓库中留下另一份行为不同的副本。
- 当前工作区副本关闭自动更新。发布独立 Git 仓库后，只需将扩展目录作为仓库根目录、更新 `homePage` 并启用 `auto_update`；运行时代码不需要再次改造。

---

## 2026-09-14 实现修订：状态证据与适用范围

本节替代旧稿中“每轮补全动机与受阻反应、统一注入边界”的实现说明。当前实现仍在正文完成后异步更新，下一次组装提示时按相关性投影。

- 保持 `schemaVersion: 2` 和原有状态字段，新增可选 `context`。旧卡、快照和聊天继续读取，不批量改写历史。
- `context.fields` 按字段路径保存来源：`basis`（明确表达、观察或推测）、逐字引文及消息 ID、`scope`、适用条件、结束依据和话题词。
- 更新器默认参考前 5 轮问答，加本次新增原文，正常为约 6 轮语境；连续用户输入和助手续写归在同一轮，而非固定把两条消息算一轮。前文预算为 24,000 字符、16 条消息；超出时先按完整轮次缩减，只有最新单轮自身超限才保留头尾并明确标记缺失。新增原文不经过此截短。只回看目标消息之前的正文，不读取后续内容。
- 前文只用于理解指代、语气、场景和因果，只有新增原文可作为新证据。字符串更新与清空都须附可核验引文。复用旧引文不能强化或清零已有状态。`think` / `thinking` 分析块（含属性、未闭合块）不作为剧情证据。
- 无变化、歧义、前因缺失时允许省略或返回 null，允许整轮零更新。新提交的 `basis=inferred` 字段直接跳过，不新增、覆盖、清空或刷新原状态证据；不确定不意味着旧事实已失效。真实的明确拒绝无需反复出现才记录。
- 旧快照中的推测仍保留供核对，但不会送回更新器充当旧事实，也不进入正文提示。旧版缺少证据的担忧、冲动和行动方案默认不注入，原记录继续可见。没有对用户历史作批量清理。
- `context.boundaries` 支持多个针对具体对象的明确限制。来源必须是明确表达；普通偏好、提醒和玩笑不能仅凭否定词成为限制。普通的当下拒绝也有效，无须上升到重大原则。
- `scene` 信息仅在记录的场景中投影；`topic` 根据最近四条正文与记录的话题词匹配；`ongoing` 可跨场景延续。无关信息暂不注入，但保留记录。当前话题匹配是保守的词语匹配，仍存在同义词/指代漏召回的限制，不能视为完美语义识别。
- 限制的撤回需单独提交 ID 和新的原文证据。对方配合、转换话题、恢复笑容，不自动取消有效要求。旧版单条限制以“范围待核验”保留，不能无依据清空。
- `responseIfBlocked` 退出正文注入与新状态生成；不预先编造用户将纠缠或角色将惩罚。真实分歧发生后记录事实与感受。
- 主提示使用简短的通用使用说明，口吻由角色卡决定，不把所有角色写成同一种伴侣。没有相关信息时允许零注入；多个已确认且相关的限制不会为字符预算被静默丢弃。
- 状态面板可展开“来源与适用范围”，区分本轮相关、其他场景/话题、推测、旧记录待核验。归档续聊保留来源并标记为归档原文，避免旧消息编号与新聊天混淆。
- 引文或字段校验失败按项隔离；转场失败时关联场景字段暂缓，边界撤回/替换作为一个事务，避免只删除旧要求却未接纳新要求。旧版缺少逐字段来源的关系概括、信号和记忆变更在核心证据失败时保留上一版本，防止无效结论绕路进入记忆。
- 保留一次针对格式/引用的纠错重试；明确标为推测时不要求重答。重试仍失败时保留先前通过核验的独立内容，不把整轮设为异常，也不合并两次互相矛盾的候选。没有任何可安全解析的结果、主体不匹配或连接失败才整轮报错。
- 部分接纳显示“状态部分更新”，全部未采纳显示“本轮状态已跳过”，不会伪装为完全成功。`delta._validation` 随快照保存跳过项目与原因，但不进入角色状态提示；全部跳过可推进检查进度而不提升状态版本。前端与单元/真实请求测试共用 `updater.js` 的解析、筛选、合并和重试路径。
- “更新详情”提供“重试未更新的回复”，只处理尚无更新检查快照的最新回复，不重新生成正文，不覆写已检查的状态。生成、归档、校准和后台更新期间禁用，防止并发覆盖。

这些结构检查只能验证引文与范围格式，不能证明模型对引文的语义判断一定正确。新用户消息引发的转场在下一次后台更新确认前，仍应由正文模型以最新输入为准；持续伤势和情绪不会按轮数自动抹除。

回归检查覆盖状态快照、删除/重生成、旧数据兼容、范围筛选、持续情绪、逐项撤回、归档来源、完整轮次窗口、超长文本、推测跳过与明确事实保留。另提供显式运行的真实请求检查：`node tests/living-state-context.live.mjs --live`，使用当前 custom 状态连接及独立虚构对话，不写入聊天。真实检查的语义断言位于格式重试之外，不会将期望答案作为重试提示交给模型；覆盖跨五轮的引用语境、提醒、沉默、明确隐私、拒绝、难过与指代不明。
...
- 只允许更新白名单字段。
- 所有场景、人物、行动和关系状态都必须附带来自本次新正文的逐字引文、依据类型与适用范围；内心意图及稳定关系判断只接受角色明确表达。
- 长期列表采用“用户确认后晋升”：除 `evidenceMessageIds` 外还必须保存逐字 `evidence`，且至少一条用户原话与所记事实有明确文本关联。仅由助手新生成的背景、回忆、承诺或伏笔不进入长期记忆。
- ID 由代码根据类型、文本和来源生成，不信任模型生成的随机 ID。
- 删除或关闭项目必须引用现有 ID，并提供明确支持完成、撤回或失效的逐字用户证据。
- `authorLocks` 中的规则不可由 Delta 修改。
- World Info 中的客观设定不可由 Living State 覆盖。
- 校验按字段执行：通过的字段可以保存，失败的字段留空或沿用最后一次已确认值，绝不把失败内容转存到关系、分数或长期列表中。
- 旧快照和缺少来源标记的项目保留供面板核对，但不再注入正文，也不反馈给更新器作为自证材料。
- 分数只供面板观察，不注入主模型；短期计划只影响紧接的一轮，避免固化为对白模板。
